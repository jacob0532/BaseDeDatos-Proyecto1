...
FROM @xmlData.nodes('Datos/Personas/Persona') xmlData(ref)
...
SELECT ref.value('@ValorDocumentoIdentidadDelCliente', 'int')
	,ref.value('@TipoCuentaId', 'int')
	,ref.value('@NumeroCuenta', 'int')
	,ref.value('@FechaCreacion', 'date')
	,ref.value('@Saldo', 'money')
FROM @xmlData.nodes('Datos/Cuentas/Cuenta') xmlData(ref)
WHERE ref.value('@NumeroCuenta', 'int') NOT IN (
		SELECT NumeroCuenta
		FROM CuentaAhorro
		)
...
SELECT ref.value('@User', 'varchar(50)')
	,ref.value('@NumeroCuenta', 'int')
FROM @xmlData.nodes('Datos/Usuarios_Ver/UsuarioPuedeVer ') xmlData(ref)
WHERE ref.value('@NumeroCuenta', 'int') NOT IN (
		SELECT NumeroCuenta
